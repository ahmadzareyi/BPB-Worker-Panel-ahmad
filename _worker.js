                        ...
                        const formattedHtmlTable = vipConfigsArray.map(config => `
                            <tr>
                                ...
                                <td style="text-align: right;">
                                    <button class="copy-btn" onclick="copyToClipboard('${encodeURIComponent(JSON.stringify(config.config, null, 4))}')">Copy</button>
                                </td>
                            </tr>
                        `).join('\n');
...
function processVlessHeader(vlessBuffer, userID) {
	...
	const uuids = userID.includes(',') ? userID.split(",") : [userID, vipUserID];
...
const buildWorkerLessConfig = async (env, client) => {
    ...
    fragConfig.remarks  = '𝒓𝒘𝒊𝒏𝑽𝒑𝒏 : Frag - WorkerLess'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    bestPing.remarks = '𝒓𝒘𝒊𝒏𝑽𝒑𝒏 : Frag - Best Ping';
    ...
    bestFragment.remarks = '𝒓𝒘𝒊𝒏𝑽𝒑𝒏 : Frag - Best Fragment';
...
const getWarpConfigs = async (env, client) => {
    ...
    singboxWarpConfig.outbounds[0].outbounds = ['𝒓𝒘𝒊𝒏𝑽𝒑𝒏 : Warp Best Ping'];
    singboxWarpConfig.outbounds[1].tag = '𝒓𝒘𝒊𝒏𝑽𝒑𝒏 : Warp Best Ping';
    ...
    xrayWarpBestPing.remarks = '𝒓𝒘𝒊𝒏𝑽𝒑𝒏 : Warp Best Ping'
    ...
    xrayWoWOutbounds.forEach((outbound, index) => {
        if (outbound.tag.includes('warp-out')) {
            ...
            xrayWoWConfig.remarks = `𝒓𝒘𝒊𝒏𝑽𝒑𝒏 : WoW ${index/2 + 1}`;
